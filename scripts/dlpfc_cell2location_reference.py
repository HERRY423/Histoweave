...
import h5py, os, tempfile, json, numpy as np, time, sys
...
cache = os.path.join(tempfile.gettempdir(), "histoweave_dlpfc_cache", "151507_filtered_feature_bc_matrix.h5")
t0 = time.time()
...
print(f"[1/5] Loaded: {X.shape[0]} spots x {X.shape[1]} genes ({X.nnz/1e6:.1f}M entries, {time.time()-t0:.1f}s)")

# ---- 2. Define brain cell-type marker genes ----
MARKER_DEFS = {
    "Neurons":        ["SNAP25", "SYT1", "GRIN1", "GAD1", "GAD2", "SLC17A7", "RGS4", "NEFL", "SST", "PVALB", "RBFOX3", "MAP2", "SYN1", "DLG4", "CAMK2A"],
    "Astrocytes":     ["GFAP", "AQP4", "ALDH1L1", "SLC1A2", "SLC1A3", "GLUL", "GJA1", "S100B", "SOX9", "GLAST"],
    "Oligodendrocytes": ["MBP", "MOBP", "PLP1", "MOG", "MAG", "SOX10", "OLIG1", "CNP", "CLDN11", "MAL"],
    "Microglia":      ["C1QA", "C1QB", "C1QC", "TREM2", "CX3CR1", "ITGAM", "P2RY12", "TMEM119", "AIF1", "CSF1R", "CD68"],
...
for ct in MARKER_DEFS:
    print(f"  {ct:<20} {len(found[ct])}/{len(MARKER_DEFS[ct])} found  "
...
for ct, markers in found.items():
    ...
    reference[ct] = dict(zip(markers, ct_profile))
...
ref_path = os.path.join(tempfile.gettempdir(), "histoweave_dlpfc_cache", "cell2location_reference.json")
...
class FakeCell2location:
    ...
    def export_posterior(self, adata, **kwargs):
        ...
        adata.obsm["q05_cell_abundance_w_sf"] = np.tile([2.0, 1.0, 0.5, 0.3, 0.1, 0.05], (adata.n_obs, 1))
        return adata
...
try:
    method = create_method("deconvolution", "cell2location",
                           max_epochs=5, use_gpu=False, n_cells_per_location=5.0,
...
print(f"  [PASS] Vitessce config generation")
