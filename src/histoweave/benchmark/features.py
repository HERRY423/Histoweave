...
DEFAULT_FEATURE_ORDER = [
    "n_obs", "n_vars", "aspect_ratio",
    "sparsity", "mean_nonzero", "library_mean", "library_cv", "expression_entropy",
    "spatial_autocorrelation", "cluster_tendency", "mean_nn_distance",
    "spatial_density_cv", "spatial_entropy",
    "effective_rank_90", "effective_rank_95", "sv_entropy",
    "n_domains", "domain_balance", "domain_spatial_coherence",
]
...
RECOMMENDATION_FEATURE_ORDER = [
    key for key in DEFAULT_FEATURE_ORDER
...
def _spatial_density_cv(coords: np.ndarray, grid_size: int = 10) -> float:
    ...
    if np.any(span < 1e-12):
        return 0.0
    bins = [
        np.linspace(mins[d] - 1e-9, maxs[d] + 1e-9, grid_size + 1)
...
def _domain_features(data: SpatialTable) -> dict[str, float]:
    ...
    if truth_col is None:
        return {"n_domains": float("nan"), "domain_balance": float("nan"),
