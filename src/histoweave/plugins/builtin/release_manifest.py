...
* **validated** — production + multi-dataset reference concordance evidence.

This manifest deliberately de-inflates maturity: teaching baselines and in-house
autoencoders are *not* production, and no method is validated without documented
multi-dataset evidence in :data:`VALIDATION_EVIDENCE`.
"""
...
# ---------------------------------------------------------------------------
# Evidence-backed validated methods (multi-dataset real-data concordance)
# ---------------------------------------------------------------------------
# Each entry documents why VALIDATED is claimed.  Keep this list short and honest.
VALIDATION_EVIDENCE: dict[str, dict[str, str]] = {
    ...
    # --- Expansion batch (histoweave.method_validation.multidataset.v1) ---
    "agglomerative": {
    ...
    # --- SOTA priority batch (histoweave.method_validation.sota_batch.v1) ---
    "spagcn": {
...
VALIDATED_METHODS = set(VALIDATION_EVIDENCE)
...
PRODUCTION_METHODS = {
    ...
    "sqrt_transform",
    "tfidf_l2",
    # Domain baselines (sklearn family) — agglomerative/birch/minibatch_kmeans
    # are elevated to VALIDATED when listed in VALIDATION_EVIDENCE.
    "agglomerative",
    "birch",
    "bisecting_kmeans",
    "dbscan",
    ...
    # Container / R bridges with declared runtimes (banksy elevated when validated)
    "banksy",
    "nnsvg",
    "r_lognorm",
}
# Methods promoted to VALIDATED must not remain sole production-only claims.
PRODUCTION_METHODS -= VALIDATED_METHODS

# ---------------------------------------------------------------------------
# Beta: real upstream wraps without full multi-dataset validation gates
# ---------------------------------------------------------------------------
BETA_METHODS = {
    "banksy_py",  # elevated to validated below via VALIDATED_METHODS
    "spectral",
    "gaussian_mixture",
    "cell2location",  # elevated when VALIDATION_EVIDENCE entry present
    "cellpose2",
    "celltypist",
    "liana_plus",
    "scanvi",
    "sctransform",
    "spatialde",  # elevated when VALIDATION_EVIDENCE entry present
    # First-class SOTA plugins (elevated to validated when evidence present)
    "spagcn",
    "graphst",
    "stagate",
    "bayesspace",
...
# Remove validated names from beta set (they are applied as VALIDATED)
BETA_METHODS -= VALIDATED_METHODS
...
EXPERIMENTAL_BASELINES = {
    "marker_deconv",
    "marker_score",
    # In-house autoencoders are research-grade representations, not field standards
...
def apply_builtin_release_manifest() -> None:
    ...
    declared = PRODUCTION_METHODS | BETA_METHODS | VALIDATED_METHODS | EXPERIMENTAL_METHODS
