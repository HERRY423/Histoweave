...
SKLEARN_METHODS = {
    "agglomerative", "birch", "bisecting_kmeans", "dbscan", "gaussian_mixture",
...
def load_5x10() -> pd.DataFrame:
    ...
    df["k_policy"] = "oracle"
    df["seeds"] = "42,1,2"
    return df[["dataset", "method_config", "family", "benchmark", "k_policy",
               "seeds", "seed", "ari", "seconds", "n_domains_truth"]]
...
def build_long() -> pd.DataFrame:
    ...
    agg = (long.groupby(["dataset", "method_config", "family", "benchmark",
                         "k_policy", "seeds"], as_index=False)
    ...
    return agg.sort_values(["dataset", "family", "benchmark", "method_config"]).reset_index(drop=True)
...
def build_matrix(long_agg: pd.DataFrame) -> tuple[pd.DataFrame, list[str]]:
    ...
    method_meta = (long_agg[["method_config", "family", "benchmark", "k_policy"]]
                   .drop_duplicates().set_index("method_config"))
    ...
    def sort_key(c):
        m = method_meta.loc[c]
        return (family_order.get(m["family"], 9),
...
def build_summary(long_agg: pd.DataFrame, cols: list[str]) -> pd.DataFrame:
    ...
    for mc in cols:
        ...
        if not aris:
            aris = [sub[sub.dataset == s]["mean_ari"].iloc[0]
                    if (sub.dataset == s).any() else float("nan") for s in SLICE_ORDER]
        ...
        rows.append({
            ...
            "mean_ari_5slices": round(mean(aris), 4) if all(pd.notna(aris)) else round(pd.Series(aris).mean(), 4),
    ...
    s = pd.DataFrame(rows)
    s["rank_within_family"] = s.groupby("family")["mean_ari_5slices"] \
...
def write_markdown(long_agg: pd.DataFrame, matrix: pd.DataFrame,
                   summary: pd.DataFrame, cols: list[str]) -> None:
    ...
    lines.append("")
    lines.append("Three independent HistoWeave benchmarks were run on **the same task** "
    ...
    lines.append("|-----------|--------|------:|----------|")
    lines.append("| `5x10_dlpfc_benchmark` | 10 sklearn baselines (expression-only) | 3 (42,1,2) | oracle-K (truth-derived n_domains) |")
    lines.append("| `5x15_spatial_aware` | 5 clusterers x 3 spatial_weight = 15 configs | 3 (42,1,2) | oracle-K |")
    lines.append("| `non_oracle_k_sota` | SpaGCN + STAGATE (GNN / graph-attention AE) | 1 (42) | oracle-K AND 3 blind estimate-K variants |")
    lines.append("")
    lines.append("> **Caveat.** The SOTA benchmark uses a single seed (42) while the "
    ...
    lines.append("")
    lines.append("Cells are mean ARI over the seeds each benchmark ran (3 for sklearn / "
    ...
    lines.append("")
    lines.append("| Rank | Method config | Family | Benchmark | K policy | Seeds | Mean ARI (5 slices) | Best ARI | Best slice | Worst ARI | Rank in family |")
    lines.append("|----:|---------------|--------|-----------|----------|------:|--------------------:|---------:|------------|----------:|---------------:|")
    ...
    for s in SLICE_ORDER:
        ...
        lines.append(f"| {s} | {SLICE_META[s]['n_domains']} | `{best_c}` | {fam} | **{best_v:.4f}** |")
    lines.append("")
    lines.append("---")
    lines.append("")
    ...
    lines.append("| Family | # configs | Best-in-family mean ARI (avg of 5 slices) | Best | Worst |")
    lines.append("|--------|----------:|------------------------------------------:|-----:|------:|")
    for f in fam_summary:
        lines.append(f"| {f['family']} | {f['n_configs']} | **{f['best_per_slice_mean_ari']:.4f}** | {f['best_per_slice_max']:.4f} | {f['best_per_slice_min']:.4f} |")
    lines.append("")
    lines.append("`best-in-family` = for each slice, take the highest mean ARI among that "
    ...
    lines.append("")
    lines.append("1. **Spatial awareness closes most of the gap to SOTA on oracle-K.** "
                 ...
                 "provide.")
    lines.append("2. **No single method dominates all 5 slices.** The per-slice winner "
    ...
    lines.append("3. **Oracle-K inflation is real and large for SOTA.** SpaGCN drops "
    ...
    lines.append("4. **Density/mode-seeking sklearn methods (dbscan, optics, "
    ...
    lines.append("5. **151669 is the hardest slice for every family** (8 domains, "
    ...
    lines.append("")
    lines.append("1. **Seed mismatch.** sklearn / spatial-aware benchmarks use 3 seeds; "
    ...
    lines.append("2. **K-policy mismatch.** sklearn and spatial-aware benchmarks are "
    ...
    lines.append("3. **Within-study only.** All 5 slices come from one study "
                 "(Maynard 2021). Cross-platform / cross-tissue transfer is not "
                 "tested here; see `benchmark_external_validation/` for that.")
    lines.append("4. **SOTA set is partial.** Only SpaGCN and STAGATE are in the "
    ...
    lines.append("5. **Ground truth is expert annotation**, itself imperfect "
    ...
    lines.append("")
    lines.append("- `parallel_experiment_table.csv` — long/tidy table, one row per "
                 "(slice, method_config) with mean/std ARI, runtime, family, benchmark, "
                 "K policy, seeds.")
    lines.append("- `parallel_experiment_matrix.csv` — wide matrix, slices x "
                 "method_configs, mean ARI (the table in section 1).")
    lines.append("- `parallel_experiment_summary.csv` — per-method aggregate with "
                 "overall and within-family ranks (the table in section 2).")
    lines.append("- `figures/parallel_heatmap.svg` / `.png` — heatmap of all "
    ...
    lines.append("```")
    lines.append("")
    lines.append("The generator reads only the three existing `benchmark_long.csv` "
    ...
    print(f"[write] {OUT/'report_parallel_experiment.md'}")


def main() -> None:
    long_agg = build_long()
    long_agg.to_csv(OUT / "parallel_experiment_table.csv", index=False)
    print(f"[write] {OUT/'parallel_experiment_table.csv'}  ({len(long_agg)} rows)")

    matrix, cols = build_matrix(long_agg)
    matrix.to_csv(OUT / "parallel_experiment_matrix.csv")
    print(f"[write] {OUT/'parallel_experiment_matrix.csv'}  ({matrix.shape[0]} slices x {matrix.shape[1]} methods)")

    summary = build_summary(long_agg, cols)
    summary.to_csv(OUT / "parallel_experiment_summary.csv", index=False)
    print(f"[write] {OUT/'parallel_experiment_summary.csv'}  ({len(summary)} methods)")

    write_markdown(long_agg, matrix, summary, cols)

    meta = long_agg[["method_config", "family", "benchmark", "k_policy"]].drop_duplicates()
    meta_map = {r["method_config"]: {"family": r["family"], "benchmark": r["benchmark"],
                                     "k_policy": r["k_policy"]}
                for _, r in meta.iterrows()}
    (OUT / "method_meta.json").write_text(json.dumps({"order": cols, "meta": meta_map},
                                                     indent=2), encoding="utf-8")
    print(f"[write] {OUT/'method_meta.json'}")

    print(f"\nMethods included: {len(cols)}")
    print(f"  sklearn baselines: {sum(1 for c in cols if meta_map[c]['family']=='sklearn')}")
    print(f"  spatial_aware configs: {sum(1 for c in cols if meta_map[c]['family']=='spatial_aware')}")
    print(f"  sota configs: {sum(1 for c in cols if meta_map[c]['family']=='sota')}")
    print(f"\nTop 5 by mean ARI over 5 slices:")
    for _, r in summary.head(5).iterrows():
        print(f"  {r['rank_overall']:>2}. {r['method_config']:<35} "
              f"family={r['family']:<13} meanARI={r['mean_ari_5slices']:.4f}")
